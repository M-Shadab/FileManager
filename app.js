...
app.get('/versions/delete/:id/:title', function(req, res){
	...
	//Deleteing explicitly :: As  gfs.files.remove() is not deleting Associated Chunks Of Files Only Deletes files
	//Chunks remains Undeleted And occupy space
	gfs.db.collection('uploadfiles.chunks').remove({files_id: mongoose.Types.ObjectId(req.params.id)}, function(err){
		if(err) {
	...
	Uploadfile.find({title: name}, function(err, files){
		...
		if(files.length >= 1){
			...
			if(files[0].count == 1 && value == -1){
				...
				return 0; //Return Container
			} else {
				files[0].count = files[0].count + value;
			}
		}	

		Uploadfile.update({title: name}, files[0], function(err){
			if(err){
				console.log('err: ', err);
			} 
